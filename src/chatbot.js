    ...
    try {
        if (Array.isArray(channel)) {
            ...
            if (channel.includes(message.channel.id)) {
                ...
                fetch(`https://api.affiliateplus.xyz/api/chatbot?message=${encodeURIComponent(message.content)}&botname=${encodeURIComponent(name)}&ownername=${dencodeURIComponent(developer)}&user=${message.author.id}`)
                    .then(res => {
                        let rep = res.json()
