...
async function suggestBtn(button, users, options = []) {
  if (button.isButton()) {
    ...
    try {
      ...
      if (options.modRole) {
        if (button.member.permissions.has('ADMINISTRATOR') || button.clicker.member.roles.has(options.modRole)) {
          if (button.member.permissions.has('ADMINISTRATOR')) {
            if (button.customId === 'no-sug') {
              ...
              button.reply({ content: 'Do you want to Deny suggestion (or) Vote ?', components: [row1], ephemeral: true })
            ...
            if (button.customId === 'agree-sug') {
              ...
              button.reply({ content: 'Do you want to Accept suggestion (or) Vote ?', components: [row1], ephemeral: true })
              let msg = await button.fetchReply()
