...
async function rps(interaction, options = []) {
    try {
        if (options.slash === true) {
            ...
            if (!opponent) return interaction.followUp({ content: 'No opponent mentioned!', ephemeral: true })
            if (opponent.user.bot) return interaction.followUp({ content: 'You can\'t play against bots', ephemeral: true })
            if (opponent.id == interaction.user.id) return interaction.followUp({ content: 'You cannot play by yourself!', ephemeral: true })
            if (opponent.user.bot) return interaction.followUp({ content: 'You cannot play against bots!', ephemeral: true })
            ...
            let opponent = interaction.mentions.members.first()
            if (!opponent) return interaction.channel.send('No opponent mentioned!')
            if (opponent.user.bot) return interaction.channel.send('You cannot play against bots')
            if (opponent.id == interaction.author.id) return interaction.channel.send('You cannot play by yourself!')
            ...
            interaction.channel.send({
                ...
                collector.on('collect', (button) => {
                    ...
                    collector.on('collect', (button) => {
                        ...
                        button.deferUpdate()
                        let embed = new Discord.MessageEmbed()
                            .setTitle(`${interaction.author.tag} VS. ${opponent.user.tag}`)
                            .setColor(options.embedColor || 0x075FFF)
                            .setFooter(foot)
                            .setDescription("Select 🪨, 📄, or ✂️")
