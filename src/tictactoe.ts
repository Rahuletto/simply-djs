    ...
    try {
      ...
      collector.on("collect", async (button) => {
        ...
        if (button.customId == "deny-ttt") {
          ...
          const idleClr = options.buttons?.idle?.style || ButtonStyle.Secondary;
          const XClr = options.buttons?.X?.style || ButtonStyle.Danger;
          const OClr = options.buttons?.O?.style || ButtonStyle.Primary;
