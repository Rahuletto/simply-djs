...
async function clickBtn(button, options = []) {
  ...
  let ol = oldembed.fields[1].value.replace('**', '')
   ...
   let dbmsg = await db.get(`givemeaway_${button.message.id}_why`)
    await button.message.channel.messages.fetch(dbmsg).then(msg => msg.delete());
  
            setTimeout(async () => {
  
  if (winner.length === 0 || winner === [] || winner[0] === '') {
  
  let ool = oldembed.fields[1].value.replace('**', '')
              let winnnerNumber = ol.replace('**', '')
  
                              const embedod = new Discord.MessageEmbed()
                                  .setTitle('No one remaining')
                                  .setColor(0xcc0000)
                                  .setDescription(`**We rerolled and no one is remaining.**\n\n` + oldembed.description.replace(`React with the buttons to enter.`, ' '))
                                  .addFields(
     ...
     .addFields(
        { name: '🏆 Winner(s):', value: `${winner}`},
         { name: '💝 People Entered', value: `***${winnerNumber}***`}
      )
      .setFooter("Giveaway Ended.")
                    ...
                    button.message.edit({ embeds: [embedd], components: [roww] })
                                  ...
                                  button.channel.send({ content: `Congrats ${winboiz}. You just won the giveaway. (after reroll)`, components: [ro] }).then(async yikes => {
                                    await db.set(`givemeaway_${button.message.id}_why`, yikes.id)
                                  })
