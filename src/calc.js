...
try{
    ...
    message.channel.send({
        ...
        function createCollector(val, result = false) {
            ...
            const filter = (button) => button.customId === 'cal' + val;
            let collect = msg.createMessageComponentCollector({ filter, componentType: 'BUTTON', time: time });

            collect.on("collect", async x => {
                if (x.user.id !== message.author.id) return x.reply({ content: '❌ Only the owner can use buttons on the message', ephemeral: true });
    ...
    console.log(`Error Occured. | calculator | Error: ${err}`)
}
